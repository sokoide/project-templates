...
Go 1.23 で追加された主要な機能を実際のコードで解説します。
...
独自のデータ構造や生成ロジックを `range` に対応させ、利用者側が内部構造を意識せずにループを回せます。
...
```go
// Before
// 1. スライスを生成して返す（メモリ負荷が高い）
// 2. Next() メソッドを持つ構造体を作り、手動で反復する
```
...
二つの値を返すイテレータを定義でき、インデックス付き反復やマップ風の操作を自然に記述できます。
...
無限に値を生成するイテレータを定義でき、`break` による中断時に自動でクリーンアップ処理（終了処理）を実行できます。
...
スライスやマップをイテレータとして扱え、呼び出し側は `range` で一貫したインターフェースを利用できます。
