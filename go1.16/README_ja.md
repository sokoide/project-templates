...
設定ファイルや SQL クエリなどのテキストファイルを外部ファイルとして管理しつつ、実行バイナリに直接組み込んで安全に利用できます。
...
画像やバイナリデータなどのアセットを、外部ツールなしで型安全にバイナリへパッキングできます。
...
```go
// Before
// go-bindata などの外部ツールを使って .go ファイルに変換する手間が必要でした。
```
...
フロントエンドの静的アセットを含むディレクトリ全体をバイナリ一つにパッキングでき、ポータビリティの高いシングルバイナリの配布が容易になります。
...
```go
// Before
// ディレクトリ構造を維持したまま埋め込むには、外部ツールで巨大なソースコードを生成する必要がありました。
```
